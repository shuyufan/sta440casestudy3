...
```{r EDA cong-not-include, include=F, fig.height = 4, fig.width = 8}
...
```{r external-validation, fig.width = 8, fig.height = 4, include = F, message = F, warning = F}
...
```{r, include = F}
cong_dist_rand_eff = readRDS("randeff_revision_cong_dist_grouped_model_output.rds")
#summary(randeff_model)
```

```{r warning=FALSE, echo = F}
cong_dist_rand_eff_waic_obj = waic(cong_dist_rand_eff)
cong_dist_rand_eff_aic = cong_dist_rand_eff_waic_obj$estimates[3,1]
#saveRDS(binary_model, "grouped_model_no_randeff_whole_dataset.rds")
```


```{r extern-validation, include = F}
...
We also fit another model with random intercept and slopes with all of the predictors for congressional districts to capture possible variation between voters in different geographic parts of North Carolina. The coefficients for the main effects (included in the Appendix, along with group-level standard deviation) look largely the same compared to the main model, with the exception that the coefficients for income are slightly different in the two models. This is perhaps because some of the variations between different income groups can be attributed to differences between counties. We did not choose this model as our main model because it has a higher WAIC (`r format(round(cong_dist_rand_eff_aic, 2), scientific=F)`) compared to the main model (`r format(round(binary_model_waic, 2), scientific=F)`). Although the RMSE when predicting on 2012 data for the random effects model (`r extern_rmse_randeff_model`) is better, we ultimately decided to keep the main effects model because of the higher level of stability in its coefficients. The main effects model also had smaller residuals and better coverage (see Appendix for residual plot and coverage plot for the random effects model). 

With the addition of random slopes, we see that for black race, congressional districts 1, 2, 8, 10, and 11 have random slopes that are significant (determined by the fact that their 95% CI's do not contain zero). Looking at both main and random effects, holding all other variables at baseline, the expected odds of voting increases by a multiplicative factor for black people from districts 8 and 10, while those odds decreases by a multiplicative factor for black people from districts 1, 2, and 11. For the 45-59 age group, the random slopes for districts 2, 3, 4, 7, 8, 9, and 11 are significant, and the expected odds of voting increases by a multiplicative factor for 45-59 year-olds from those districts, holding all else at baseline. For the Republican party, random slopes for districts 2, 4, 5, 8, 10, and 13 are significant, and the expected odds of voting increases by a multiplicative factor for Republicans in those districts, holding all else at baseline.
...
```{r EDA cong, echo=F, fig.height = 4, fig.width = 8}
ggplot(voter_cong, aes(x = cong_dist_abbrv, y = voterpercent)) + geom_boxplot(aes(fill = party_cd)) + labs(y="Turnout Percentage", x = "Congressional District", title = "Voter Turnout for Different Congressional Districts", fill = "Party") +theme(plot.title = element_text(hjust = 0.5,size = 8),axis.title=element_text(size=8),legend.key.size = unit(0.5,"line"))
```
...
priors <- c(set_prior("normal(0,1.5)", class = "Intercept"),
            set_prior("normal(0,1.5)", class = "b"),
            set_prior("normal(0.5,1.5)", class = "b", coef = "age_binned30M44"),
            set_prior("normal(0.75,1.5)", class = "b", coef = "age_binned45M59"),
            set_prior("normal(1,1.5)", class = "b", coef = "age_binned60P"),
            set_prior("normal(-0.5,1.5)", class = "b", coef = "gender_codeM" ),
            set_prior("normal(-0.5,1.5)", class = "b", coef = "race_codeBlack" ),
            set_prior("normal(-1,1.5)", class = "b", coef = "race_codeOther" ),
            set_prior("normal(-0.5,1.5)", class = "b", coef = "party_cdOther" ),
            set_prior("normal(0.5,1.5)", class = "b", coef = "med_inc_binned46864M52798" ),
            set_prior("normal(0.5,1.5)", class = "b", coef = "med_inc_binned52798M64509" ),
            set_prior("normal(1,1.5)", class = "b", coef = "med_inc_binned>64509" ))
...
coef_table3 = round(fixef(cong_dist_rand_eff),2)
...
kable(coef_table3) 
#ranef(randeff_model)
group_sd_obj = VarCorr(cong_dist_rand_eff)
group_sd = group_sd_obj$cong_dist_abbrv$sd[1]
```

```{r, echo = F, message = F, warning = F}
rand_slopes = ranef(cong_dist_rand_eff)
kable(cbind(seq(1,13),round(rand_slopes$cong_dist_abbrv[,,"race_codeBlack"],2)), caption = "Race Black Random Slope")
kable(cbind(seq(1,13),round(rand_slopes$cong_dist_abbrv[,,"age_binned45M59"],2)), caption = "Age 45-59 Random Slope")
kable(cbind(seq(1,13),round(rand_slopes$cong_dist_abbrv[,,"party_cdREP"],2)), caption = "Party Republican Random Slope")
```

```{r, echo = F, fig.width = 8, fig.height = 4, fig.cap = "Coverage Plot for Random Effects Model", message = F, warning = F}
voter_grouped = voter_hist_districts_resp %>%
  mutate(med_inc_binned = as.character(med_inc_binned)) %>%
  mutate(med_inc_binned = case_when(
    as.character(med_inc_binned) == "46864 - 52798" ~ '46864 _ 52798',
    as.character(med_inc_binned) == "52798 - 64509" ~ '52798 _ 64509',
    TRUE ~ med_inc_binned)) %>%
  group_by(age_binned, gender_code, party_cd, race_code, med_inc_binned, cong_dist_abbrv) %>%
  summarise(n = n(), votes = sum(vote_or_not))
...
d <-
  voter_grouped %>% 
  ungroup() %>% 
  mutate(case = factor(1:nrow(voter_grouped))) 
...
```{r resids-randeff, echo = F, fig.width = 8, fig.height = 4, fig.cap = "Random Effects Model Residual Plot", message = F, warning = F}
# Residuals
...
```{r include=FALSE, cache = T, message = F, warning = F}
