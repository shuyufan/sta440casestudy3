...
voter_small = readRDS("ncvoter_Statewide_small.rds") %>%
  ...
  filter(birth_age <= 116) %>% # oldest NC resident is 116 years old
  dplyr::select(-birth_year, -status_cd, -reason_cd)

# need this because there is 0000000  
...
hist_small = readRDS("ncvhis_Statewide_small.rds") %>%
  #mutate(unique_id = paste(county_desc, voter_reg_num, sep = "_")) %>% 
  filter(election_desc == "11/08/2016 GENERAL") %>% # only looking at 2016 general election
  dplyr::select(-election_lbl, -voted_party_cd, -pct_label, -voted_county_id, -vtd_label)

# remove duplicates in hist_small
hist_small_not_dup =  hist_small[!duplicated(hist_small$ncid),]

# process 2018 median income data
...
voter_hist_districts_resp_small = sample_n(voter_hist_districts_resp, 1000000) 
