...
plot_usmap(data = pres_state_df, values = "pwin", color = "black") + 
  scale_fill_gradient2(low = "red", mid = "white", high = "blue", midpoint = 0.5,
                       aesthetics = "fill", name = "Predicted Probability of Win for Biden", label = scales::comma) + 
  theme(legend.position = "right")
```
...
From the table of predicted Republican vote share in the House elections for each district, it appears that the Republican candidate will win in districts 3, 5, 7, 8, 9, 10, 11, and 13 (where the lower bounds of the 95% confidence intervals for predicted vote share are also all above 50).
...
ggplot() +
  geom_polygon(data = nc_cong_dist_vs_df, 
               ...
               , color="white") +
      geom_text(data=rnames, aes(long, lat, label = id), size=3) +

    # geom_text(data=nc_cong_dist_vs_df,
    #         aes(x = long, y = lat, group = group, label = dist_id),
    #         color= "black", size=3, fontface = 2,
    #         hjust = 0.5, vjust = 0.5) +
