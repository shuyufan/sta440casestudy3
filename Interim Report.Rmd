...
We propose a model with the folloing form: 
...
Due to the lack of strong prior knowledge, we will use `brm` function's default priors: flat prior for all the $\beta$ coefficients and a Student t distribution with mean = 0, sigma = 2.5 and degree of freedom = 3. i.e. $\alpha \sim Student_3(0,2.5)$ and $\beta \sim Unif(0,1)$
...
Figure \ref{ppc} displays the posterior predictive checks for our model. The plots show that our simulated distribution is similar to the observed distribution. Our simulations also produces few outliers. It appears that the true values are covered by the 95% confidence intervals of the predicted values, indicating model fit.
...
We fit a Bayesian logistic regression model to model the binary outcome of whether or not a demographic subgroup, defined by their race, gender, party affiliation, age, and median household income (such as white Female Democrats between 30-44 years of age whose median household income is greater than $64,509) votes. Based on results from literature review, we added interactions between gender and party, race and party, age and gender, age and party
no random effect to our model. MCMC diagnostics and model diagnostics do not indicate lack of fit. 
\newline
All of the demographic variables, as well as the interactions, were significant except for the interaction between unspecified gender and the "other" party category. From the plots of the marginal effects of the covariates, holding median household the $< \$46,864$ category and holding age at the 18-29 level, we found that people whose gender is female have a higher expected probability of voting. For those with the same median household income as above and are female, we note that among white voters, Republicans are more likely to vote that Democrats, while the opposite is true among black voters. We also found that for those whose median household income falls in the highest category ($> \$64,509$, 75% quantile for median household incomes by county), the expected probability of voting is larger relative to the other income categories.
\newline
The biggest strength of this study is its focus grouping data and using it to analyze voting patterns, so that the results can directly be used for future analysis on the same demographic groups and election candidates can use this result to adjust their resource allocation during campaigns. Many key limitations are already addressed previously. Others include the weaknesses of the data set - the voting patterns in 2016 will not completely match that of 2020 even though Trump has been the Republican candidate for both races as the Democratic candidate will mobilize voters differently and the political landscape has changed from 2016 to 2020 (Weinschenk et al.). In addition, grouping also prevents us from analyzing individual-level demographic variations and predictors not yet included in our analysis. As voter turnout is a very complicated subject and a lot of predictors might be relevant in shaping voting patterns, we might be oversimplifying the study with the number of predictors we have on group levels.
\newline
