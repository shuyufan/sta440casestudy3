...
```{r}
betas_day_of_election = sapply(1:14, function(i) {paste0("theta.", i, ".1.")} )
parameters = c(betas_day_of_election) # what else do we want to look at?

#traceplot(jags_sims_mv,  varname = "theta")

tp_vars = data.frame(jags_sims_mv$BUGSoutput$sims.matrix)[,parameters]
# colnames(tp_vars) <- gsub("^.*\\.","", colnames(tp_vars))
